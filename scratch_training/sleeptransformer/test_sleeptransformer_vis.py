...
list_of_files = []
file_sizes = []
with open(os.path.abspath(FLAGS.eeg_test_data)) as f:
    lines = f.readlines()
    for l in lines:
        print(l)
        items = l.split()
...
for lof, fs in zip(list_of_files, file_sizes):
    n = os.path.basename(lof).split('.')[0]
    
    test_gen_wrapper = DataGeneratorWrapper(
        eeg_list_of_files=[lof], 
        file_sizes=[fs],
        eeg_filelist=None,
    ...
    with tf.Graph().as_default():
        ...
        with sess.as_default():
            ...
            def dev_step(x_batch, y_batch):
                ...
                output_loss, total_loss, yhat, score, attention_weight, attention_out = sess.run(
                    [net.output_loss, net.loss, net.predictions, net.scores, net.attention_weight, net.attention_out],
                    feed_dict
                )
                
                return output_loss, total_loss, yhat, score, attention_weight, attention_out, x_batch  # Return attention weights and inputs
            ...
            def evaluate(gen_wrapper):
                ...
                attention_weights = np.zeros([N, config.epoch_seq_len, config.frame_seq_len])
                inputs = np.zeros([N, config.epoch_seq_len, config.frame_seq_len, config.ndim, config.nchannel])
                ...
                for data_fold in range(config.num_fold_testing_data):
                    ...
                    yhat_, score_, attention_weights_, inputs_, output_loss_, total_loss_ = _evaluate(gen_wrapper.gen)
                    ...
                    attention_weights[count:count + len(gen_wrapper.gen.data_index)] = attention_weights_
                    inputs[count:count + len(gen_wrapper.gen.data_index)] = inputs_
                ...
                return test_acc, yhat, score, attention_weights, inputs, y, output_loss, total_loss
            ...
            def _evaluate(gen):
                ...
                attention_weights = np.zeros([len(gen.data_index), config.epoch_seq_len, config.frame_seq_len])  # Initialize array
                inputs = np.zeros([len(gen.data_index), config.epoch_seq_len, config.frame_seq_len, config.ndim, config.nchannel])  # Initialize array
                ...
                while test_step < num_batch_per_epoch:
                    ...
                    output_loss_, total_loss_, yhat_, score_, attention_weight_, attention_out_, input_x_ = dev_step(x_batch, y_batch)
                    attention_weight_ = attention_weight_.reshape(-1, config.epoch_seq_len, config.frame_seq_len)
                    ...
                    attention_weights[(test_step - 1) * factor * config.batch_size: test_step * factor * config.batch_size] = attention_weight_
                    inputs[(test_step - 1) * factor * config.batch_size: test_step * factor * config.batch_size] = input_x_
                    test_step += 1
                if(gen.pointer < len(gen.data_index)):
                    actual_len, x_batch, y_batch, label_batch_ = gen.rest_batch(factor*config.batch_size)
                    output_loss_, total_loss_, yhat_, score_, attention_weight_, input_x_ = dev_step(x_batch, y_batch)
                    attention_weight_ = attention_weight_.reshape(-1, config.epoch_seq_len, config.frame_seq_len)
                    ...
                    attention_weights[(test_step - 1) * factor * config.batch_size: len(gen.data_index)] = attention_weight_
                    inputs[(test_step - 1) * factor * config.batch_size: len(gen.data_index)] = input_x_
                    output_loss += output_loss_
                    total_loss += total_loss_
                yhat = yhat + 1 # make label starting from 1 rather than 0

                return yhat, score, attention_weights, inputs, output_loss, total_loss

            # Final evaluation
            start_time = time.time()
            test_acc, test_yhat, test_score, test_attention_weights, test_inputs, test_y, test_output_loss, test_total_loss = evaluate(gen_wrapper=test_gen_wrapper)
            end_time = time.time()

            # Save the results, including attention_weights and inputs
            hdf5storage.savemat(os.path.join(out_path, f"{n}_test.mat"),
                {
                    'yhat': test_yhat,
                    'acc': test_acc,
                    'score': test_score,
                    'attention_weight': test_attention_weights,
                    'input_x': test_inputs,
                    'ytrue': test_y, 
