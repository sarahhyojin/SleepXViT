...
from dataset_concat import CustomImageDataset
...
def model_train(model, data_loader, loss_fn, optimizer, device, num_seq):
    ...
    return all_labels, all_preds, attention_scores_list, avg_loss, acc, f1
...
if __name__ == '__main__':
    ...
    for epoch in range(0, num_epochs, 1): # evaluate every epoch
        train_labels, train_preds, train_weights, train_loss, train_acc, train_f1 = model_train(Net, train_dataloader, loss_fn, optimizer, device, num_seq)
        val_labels, val_preds, val_weights, val_loss, val_acc, val_f1 = model_evaluate(Net, eval_dataloader, loss_fn, device, num_seq)
